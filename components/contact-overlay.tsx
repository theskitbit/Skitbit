      ...
      {isOpen && (
        <motion.div className="fixed inset-0 z-50 bg-[#F6F7F2] overflow-y-auto text-foreground flex flex-col" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          <div className="flex justify-between items-center px-6 py-6 sm:py-8 max-w-5xl w-full mx-auto shrink-0">
            ...
            <button type="button" onClick={onClose} className="w-10 h-10 rounded-full border border-black/5 flex items-center justify-center hover:bg-black/5 text-slate-400 transition-colors" aria-label="Close dialog">✕</button>
          </div>
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="flex flex-wrap gap-3">
                            {needsOptions.map((o) => {
                              ...
                              return (
                                <motion.button type="button" key={o} onClick={() => setData({ ...data, needs: active ? data.needs.filter(n => n !== o) : [...data.needs, o] })} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={`flex flex-auto sm:flex-initial justify-center items-center px-5 py-3.5 rounded-full border cursor-pointer transition-all text-sm font-medium ${active ? 'bg-[#D4F05A] text-[#0B1A28] border-[#D4F05A]' : 'border-black/10 text-slate-600 bg-transparent'}`}>
                                  {active && <CheckIcon />}
                                  <span className="whitespace-nowrap">{o}</span>
                                </motion.button>
                              )
