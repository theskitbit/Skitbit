...
export default async function WorkDetailPage({
  ...
  return (
    <main className="min-h-screen bg-[#07111f] text-[#f6f7ed]">
      <Header />
      <article className="mx-auto max-w-7xl px-5 pb-20 pt-32 sm:px-8 md:pb-28 md:pt-40">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] lg:items-start lg:gap-20">
          <header className="lg:sticky lg:top-28">
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.2em] text-[#dff04a]">Selected work</p>
            <h1 className="max-w-2xl text-5xl font-semibold tracking-[-0.06em] sm:text-7xl">{project.title}</h1>
            <p className="mt-7 max-w-xl text-lg leading-relaxed text-[#f6f7ed]/65">{project.description}</p>
            <div className="mt-8 flex flex-wrap gap-2">
              {[project.formatTag, project.fidelityTag, ...project.industries].map((tag) => (
                <span key={tag} className="rounded-full border border-white/15 bg-white/[0.08] px-3 py-1.5 text-xs font-medium text-white/75 backdrop-blur-md">{tag}</span>
              ))}
            </div>
          </header>

          <div className="overflow-hidden rounded-[1.5rem] border border-white/15 bg-white/[0.06] p-2 shadow-2xl shadow-black/30 backdrop-blur-xl">
            {isVideo ? (
              <video src={project.mediaUrl} poster={project.posterUrl} controls playsInline className="max-h-[78vh] w-full rounded-[1.1rem] object-contain" />
            ) : (
              <img src={project.mediaUrl} alt={project.title} className="max-h-[78vh] w-full rounded-[1.1rem] object-contain" />
            )}
          </div>
        </div>
      </article>
